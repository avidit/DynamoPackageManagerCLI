...
const builtInPackagePath = `${process.env.APPDATA}\\Dynamo\\${DYNAMO_HOST}\\${DYNAMO_VERSION}`
...
const findAllInstalledPackages = async () => {
  try {
    ...
    return customPackageFolders
      ...
      .map((folder) => {
        return folder === builtInPackagePath
...
const downloadPackage = async (query, version) => {
  try {
    const preferences = await readPreferences()
    const preferenceSettings = preferences.PreferenceSettings
    let packagePath = preferenceSettings.SelectedPackagePathForInstall[0]
    if (packagePath === builtInPackagePath) {
      packagePath = path.join(builtInPackagePath, 'packages')
    }
    const data = await getPackage(query)
...
const deletePackage = async (name) => {
  try {
    ...
    if (res) {
      ...
      rmSync(res.location, { recursive: true })
    } else {
